...
def get_tics(pth):
    ...
    tic_exchange_dic = {}  # get an empty dictionary to contain the tickers' information
    with open(pth,mode='r') as file:                        # Open and read the 'TICKERS.txt'
        for line in file:
            line = line.strip()                             # No spaces
            line = line.lower()                             # All characters in lower case
            exchange,ticker = line.split(sep='=')
            ticker= ticker.strip('"')                       # No quotation marks
            exchange = exchange.strip('"')
            if ticker.isalpha() and exchange.isalpha():     # Only contain alphabetical characters
                if ticker and exchange:                     # No empty tickers or exchanges
...
def read_dat(tic):
    ...
    """
    sub_pth=os.path.join(DATDIR,f"{tic}_prc.dat")       # Create a variable with the location of the relevant file
    lst=[]                                              # Create an empty lst to contain the text
    with open(sub_pth,mode='r') as sub:
        for line in sub:
            line=line.strip('\n').strip('\r').strip()   # Remove newline characters
            lst.append(line)
    return lst
...
def line_to_dict(line):
    ...
    dic={}
    start=0
    for keys,colwidth in COLWIDTHS.items():       # Extract the correct value
        end=start+colwidth
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is not None:
        ...
        for ticker in tickers_lst:
            if ticker not in tic_exchange_dic.keys():
                raise Exception
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    if col_lst is None:
        col_lst=COLUMNS
    verify_tickers(tic_exchange_dic,tickers_lst)            # To check if tickers_lst contains any invalid tickers
    verify_cols(col_lst)                                    # To check if col_lst contains any invalid column names


    data_dict={}
    for ticker in tickers_lst:
        data={'exchange':tic_exchange_dic[ticker],'data':[]}
        lines=read_dat(ticker)                              # Get ticker's information
        for line in lines:
            l_dic=line_to_dict(line)
            subline_dic={col:l_dic[col] for col in col_lst} # Loop to get inormation in dictionary format
            data['data'].append(subline_dic)
...
def create_json(data_dict, pth):
    ...
    """
    with open(pth,mode='w') as json_file :
        json.dump(data_dict,json_file,indent=2)            # Use indent to improve the readability of the json file
